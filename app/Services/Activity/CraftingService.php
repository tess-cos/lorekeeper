...
class CraftingService extends Service
{
    ...
    public function act($activity, $data, $user)
    {
        ...
        try {
            ...
            if ($activity->limit) {
                if (!$activity->checkLimit($activity, $user)) {
            ...
            if ($recipe->ingredients->where('ingredient_type', 'Item')->count() > 0 && !$ingredients) {
                throw new \Exception('Insufficient subjects.');
            }
            ...
            foreach ($ingredients as $id => $quantity) {
                ...
                if (!$service->debitStack($user, 'Activity', ['data' => 'Used in ' . $activity->name . ''], $stack, $quantity)) {
            ...
            foreach ($currency_ingredients as $ingredient) {
                if (!$service->debitCurrency($user, null, 'Activity', 'Used in ' . $activity->name . '', Currency::find($ingredient->data[0]), $ingredient->quantity)) {
            ...
            if (!$rewards = fillUserAssets($recipe->rewardItems, null, $user, $logType, $rewardData)) {
    ...
    public function checkRecipe($user, $recipe)
    {
        ...
        foreach ($recipe->ingredients as $ingredient) {
            ...
            if (intval($userOwned) < $ingredient->quantity) {
                $completed = false;
            }
