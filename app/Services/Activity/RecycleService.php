...
class RecycleService extends Service
{
    ...
    public function act($activity, $data, $user)
    {
        ...
        try {
            ...
            if (array_sum($data['stack_quantity']) % $activity->data->quantity !== 0) {
                throw new \Exception('Please select the correct number of items to turn in. It should be divisible by ' . $activity->data->quantity);
            }
            ...
            for ($i = 0; $i < $rewardCount; $i++) {
                ...
                if (!isset($allRewards)) {
                    ...
                    $allRewards = mergeAssetsArrays($allRewards, $rewards);
                }
