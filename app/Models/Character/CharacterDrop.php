...
class CharacterDrop extends Model
{
    ...
    **********************************************************************************************/
    ...
    {
        return $query->whereNotIn('character_id', Character::where('is_myo_slot', 1)->pluck('id')->toArray())->whereIn('drop_id', CharacterDropData::where('is_active', 1)->pluck('id')->toArray())->where('next_day', '<', Carbon::now());
    }
