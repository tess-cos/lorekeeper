...
class User extends Authenticatable implements MustVerifyEmail
{
     ...
     */
    protected $fillable = [
        'name', 'alias', 'rank_id', 'email', 'password', 'is_news_unread', 'is_banned', 'has_alias', 'avatar', 'is_sales_unread', 'theme_id', 'decorator_theme_id', 'birthday', 'home_id', 'home_changed', 'faction_id', 'faction_changed', 'has_accepted_terms','is_polls_unread'
    ];
    ...
    **********************************************************************************************/
    ...
    {
        return '<small><a href="'. $this->url .'" style="background-color: #f4e3e6; text-transform: capitalize;" class="btn btn-dark btn-sm aca-4"'.($this->rank->color ? 'style="background-color: #'.$this->rank->color.'!important;color:#000!important;"' : '').'><i class="'.($this->rank->icon ? $this->rank->icon : 'fas fa-user').' mr-1" style="opacity: 50%;"></i>'. $this->name .'</a></small>';
    }
    ...
    **********************************************************************************************/
    ...
    public function getFetchCooldownAttribute()
    {
        ...
        $log = ItemLog::where('sender_id', $this->id)->where('log_type', 'Turned in for Help Wanted request')->orderBy('id', 'DESC')->first();
