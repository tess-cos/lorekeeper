...
@else
    <div class="text-center mb-3">
        @if ($stock->stock_type == 'Pet')
            <div class="mb-1"><a href="{{ $stock->item->idUrl }}"><img src="{{ $stock->item->VariantImage($stock->variant_id) }}" alt="{{ $stock->item->VariantName($stock->variant_id) }}" /></a></div>
            <div><a href="{{ $stock->item->idUrl }}"><strong>{{ $stock->item->VariantName($stock->variant_id) }}</strong></a></div>
        @else
    ...
    @if ($stock->item->parsed_description)
        <div class="mb-2">
            <a data-toggle="collapse" href="#itemDescription" class="h5">Description <i
