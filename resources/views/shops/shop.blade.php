                        ...
                        <div class="row mb-3">
                            @foreach($chunk as $item)
                                ...
                                @include('widgets._wishlist_check')
                                        ...
                                        @if($item->pivot->purchase_limit) <div class="text-success">Max {{ $item->pivot->purchase_limit }} @if($item->pivot->purchase_limit_timeframe !== 'lifetime') {{ $item->pivot->purchase_limit_timeframe }} @endif per user</div> @endif
                                        @if($item->pivot->disallow_transfer) <div class="text-success">Cannot be transferred after purchase</div> @endif
                                    </div>
                                </div>
                            @endforeach
                        </div>
