...
<h1 class="ct mb-0">
    @if(Config::get('lorekeeper.extensions.character_status_badges'))
        ...
        <div class="float-right">
            <span class="btn {{ $character->is_trading ? 'badge-success' : 'badge-danger' }} float-right ml-2" data-toggle="tooltip" title="{{ $character->is_trading ? 'OPEN for sale and trade offers.' : 'CLOSED for sale and trade offers.' }}"><i class="fas fa-comments-dollar" style="padding: 2.5px !important; font-size: 12pt;"></i></span>
            @if(!$character->is_myo_slot)
