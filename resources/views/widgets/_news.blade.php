    ...
    <div class="card-body pt-0" style="border: 0px; margin-top: 5px;">
        @if($newses->count())
            @foreach($newses as $news)
                <div class="aca-3" style="padding: 5px; padding-top: 15px; background-color: #fdfdfd; color: #6a6a6a; border-bottom: 10px solid #fcfcfc;">
                    ...
                    @if($textPreview)
                        <p class="pl-3 ma">{!! substr(strip_tags(str_replace("<br />", "&nbsp;", $news->parsed_text)), 0, 85) !!}... <a class="cc-3" style="font-weight: bold; font-family: Poppins, serif;" href="{!! $news->url !!}">read more <i class="fas fa-arrow-right"></i></a></p>
                    @endif
                </div>
            @endforeach
        @else
