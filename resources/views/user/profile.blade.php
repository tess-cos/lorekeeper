...
@if(Settings::get('event_teams') && $user->settings->team)
        <div class="text-center ttm" style="padding: 20px; width: 150px; margin: auto; border-radius: 10px; margin-bottom: 8px; margin-top: 4px;">
            <a href="{{ url('event-tracking') }}">
                ...
                @endif
                <h5 class="ttmt">{{ $user->settings->team->name }}</h5>
            </a>
        </div>
    @endif
...
@if(isset($user->profile->parsed_text))
    <div class="card mb-3" style="clear:both; border: 0px; padding: 0px;">
        <div class="card-body bb2-2" style="background: #ccc;">
                    ...
                    <div class="row">
                        @foreach($pets as $pet)
                            <div class="col profile-inventory-item">
                                <a href="#" class="inventory-stack"><img src="{{ $pet->variantimage($pet->pivot->variant_id) }}" style="max-height: 130px; max-width: 130px;" data-toggle="tooltip" title="{{ $pet->name }}" alt="{{ $pet->name }}" />
                            </div>
                        @endforeach
                    </div>
                @else
